...
export default function ClientInfo({clientId}) {
  ...
  console.log(dataOutstanding,"data");
  useEffect(() => {
  ...
  return (
    <Row gutter={24} tw="mb-1.5 max-w-full">
      ...
      {statusOutstanding === "loading" && (
       ...
       {statusOutstanding === "success" && <CardDetail
          bordered={true}
        ...
        >
              ...
              <div className="h-full col-content p-20">
                ...
                >
                  <div tw="flex justify-between mb-5">
                    ...
                    <span onClick={()=>history.push('/invoices/outstanding-balance')} tw="text-primary text-2xl font-bold cursor-pointer">{numberWithDot(dataOutstanding?.outstanding_invoices?.total)}</span>
                  </div>
                  <div >
                  {/* <DashboardChart/> */}
                  <BarChartClient data={dataOutstanding}/>
          ...
          <div tw="flex justify-center mt-10">
            <div tw="grid justify-items-center">
              <FileDoneOutlined />
              <span>{numberWithDot(dataOutstanding?.outstanding_draft.total)}</span>
              <p>in draft</p>
            </div>     
          </div>
        </CardDetail>}
