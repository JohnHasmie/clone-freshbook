...
export default function ClientsDeleted() {
  ...
  const handleOk = () => {
    if (isType === "undelete") {
      ...
      mutationRestoreBatch.mutate({ids:selectedRowKeys})}else{
  ...
  const mutationRestoreBatch = useMutation(
    async (data) => {
      return axios.post(`clients/batch-restore`, data).then((res) => res.data);
    },
    {
      onSuccess: () => {
        setTimeout(() => {
