...
export default function ClientsDraft() {
  ...
  const { data: dataDraft, status: statusDraft } = useQuery(
    ["draft-listing", filterDraft],
    async (key) =>
      axios
  ...
  // const data =
  // statusDraft === "success" &&
  // dataDraft?.outstanding_invoices?.invoices?.map((item) => ({
  //   key: item?.id,
  ...
  const defaultFooter = () => (
    <div tw="text-right text-base">
      Total In Draft: {filterDraft.currency === "USD" ? "$" : "£"}
      {data &&
        getTotal(
          data?.map((x) => {
...

    {
      ...
      render: (text, record) => (
        <div tw="z-50">
          ...
          <span>{`${filterDraft.currency === "USD" ? "$" : "£"} ${numberWithDot(
