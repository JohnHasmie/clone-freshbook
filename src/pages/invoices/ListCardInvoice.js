          ...
          ?.map((item, i) => (
            <Link key={item?.id} to={`/invoices/${item?.id}/invoice-detail`} tw="mr-5">
              <CardInvoice
                ...
                tw="w-44"
                actions={[
                  <span tw="font-bold text-center">{item?.status}</span>,
                ]}
              >
                <span tw="text-xs text-gray-600 mb-2">{item?.code}</span>
                <div tw="flex flex-col mb-10 ">
                  <span tw="font-bold text-gray-600">
                    {truncate(item?.client?.company_name, 18)}
                  </span>
                  <span tw="text-xs text-gray-600">
                    {moment(item?.issued_at).format("MM/DD/YYYY")}
                  </span>
                </div>
                <Divider />
