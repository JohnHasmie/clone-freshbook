...
export default function RecurringRevenue() {
  ...
  // ]);
  const [headers, setHeaders] = useState(["Income Summary"]);
  ...
  const mutation = useMutation(
    async (data) => {
      return axios
        .post("reports/revenue-annual/send", data)
        .then((res) => res.data);
    },
    {
      onSuccess: (res) => {
  ...
  let newHeaders =
    statusRevenue === "success" &&
    dataRevenue?.data?.record?.map((item) => item.month);

  let newDataRecurring =
    statusRevenue === "success" &&
    dataRevenue?.data?.record?.map((item) => item.recurring);
  let newDataNonRecurring =
    statusRevenue === "success" &&
    dataRevenue?.data?.record?.map((item) => item.non_recurring);
  let newDataTotal =
    statusRevenue === "success" &&
    dataRevenue?.data?.record?.map((item) => item.total);
  ...
  useEffect(() => {
    if (statusRevenue === "success") {
      ...
      setData([
        ["Recurring Income", ...newDataRecurring],
        ["Non-Recurring Invoices", ...newDataNonRecurring],
        ["Total", ...newDataTotal],
      ]);
  ...
  console.log("journey", newDataRecurring);
  const csvReport = {
    data: data,
    headers: headers,
  ...
  const generatePdf = () => {
    ...
    doc.save("recurring_revenue_annual.pdf");
  };
        ...
        <div ref={myRef} tw="md:col-span-9 mb-10 mt-10 md:mt-2">
          <CardReporting>
            ...
            <div tw="my-3 flex flex-col">
              ...
              <span tw="text-sm text-gray-600">
                For {moment(new Date(filter.start_at)).format("MMM DD, YYYY")} -{" "}
                {moment(new Date(filter.finish_at)).format("MMM DD, YYYY")}
              </span>
            </div>
                  ...
                  <tbody tw="overflow-scroll">
                    ...
                    <tr tw="text-left text-xs border-b border-dotted border-gray-200">
                      ...
                      {dataRevenue?.data?.record?.map((item, i) => (
                        <td>
                          {filter?.currency == "GBP" ? "£" : "$"}
                          {item.recurring && numberWithDot(item.recurring)}
                        </td>
                      ))}
                      {/* {items.map(item=>(
                    ...
                    <tr tw="text-left text-xs border-b border-dotted  border-gray-200">
                      ...
                      {dataRevenue?.data?.record?.map((item, i) => (
                        <td>
                          {filter?.currency == "GBP" ? "£" : "$"}
                          {item.non_recurring &&
                            numberWithDot(item.non_recurring)}
                        </td>
                      ))}
                      {/* {items.map(item=>(
...
<td>{filter?.currency == "GBP" ? '£' : "$"}{item}</td>))} */}
        ...
        <Filter
          Filtering={FilterRecurringRevenue}
          setOpen={setOpen}
          open={open}
        />
      </div>
