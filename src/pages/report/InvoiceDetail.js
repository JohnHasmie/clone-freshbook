...
import SendEmail, { SendEmailDefault, SendEmailDefaultWithCsv } from '../../components/Reports/SendEmail';
...
export default function InvoiceDetail() {
 ...
 end_date:moment().endOf('year').format("MM/DD/YYYY"),
  ...
  const FilterInvoiceDetail = (
    <div>
      <div tw="flex justify-between ">
        ...
        <p tw="text-base text-primary cursor-pointer" 
        onClick={()=>setFilter({
          limit: 10,
          page: 1,
       client_id:"",
      ...
      <Form
        ...
        fields={[
          ...
          : "" },
          { name: ["end_date"], value: filter.end_date
  ...
  const mutation = useMutation(
    async (data) => {
      return axios.post(`invoices/detail/${dataInvoices?.data[0]?.id}/send`, data).then((res) => res.data);
    },
    {
      onSuccess: (res) => {
  ...
  return (
    <div tw="max-w-screen-lg mx-auto">
      <div tw="grid grid-cols-1 gap-y-2 md:grid-cols-2 mx-5">
          ...
          <Popover placement="bottom" content={<SendEmailDefaultWithCsv  csv={{ ...csvReport }} hide={hide} dataClients={dataClients} user={newUser} mutation={mutation}/>} trigger="click" visible={clicked}  onVisibleChange={handleClickChange}>
      ...
      {status === "success" &&      <span tw="text-sm text-gray-600">Total Invoiced: { getTotal(dataInvoices?.data?.map((item)=>{
    const splitAmount=item?.total?.split(".")
    return parseInt(splitAmount[0]);
  }))} ({filter.currency})</span>}
            <span tw="text-sm text-gray-600">
              For {moment(new Date(filter.start_date)).format("MMM DD, YYYY")} - {moment(new Date(filter.end_date)).format("MMM DD, YYYY")}
            </span>
          </div>
              ...
              <thead className="theadCustom">
                <tr>
                  <th tw="pt-12 text-left py-4 ">
                    ...
                    <span tw="text-primary ml-1">    {dataInvoices?.data[0]?.client?.company_name}</span>
                  </th>
                </tr>
              </thead>
              <tbody>
                ...
                <tr tw="border-b border-dotted">
                  ...
                  <td tw="text-right"> { getTotal(dataInvoices?.data?.map((item)=>{
    const splitAmount=item?.total?.split(".")
    return parseInt(splitAmount[0]);
  }))} </td>
                </tr>
              ...
              <tfoot>
                <tr className="double">
                  ...
                  <td tw="pt-3  flex flex-col items-end ">
                    ...
                    <span tw="text-gray-600 text-right uppercase">{filter.currency}</span>
                  </td>
                </tr>
              </tfoot>
            </table>}
          ...
          <table tw="mt-10" key={i}>
            <tbody>
              ...
              <tr tw="border-b border-gray-300">
                <th tw=" text-left ">
                  <span>Issued: {moment(new Date(item.issued_at)).format("MMM DD, YYYY")}</span>
                </th>
                <th></th>
                <th></th>
                <th></th>
